...
export default function App() {
  ...
  //Generates random number on startup
  const reset = () => {
  ...
  //Checks if the users guess was higher, lower or equal to the randomly generated number, providing
  //feedback depending on the outcome
  const checkingGuess = (guess:any, guessNumb:any) => {
    
    //Increments user guess count
    guessNumb++
    setNumberOfGuess(guessNumb)

    //Checks users guess
    if (guess > randomNumber) {
      setUserFeedback("Guess Lower")
    } 
    if (guess < randomNumber){
      setUserFeedback("Guess Higher")
    }
    if (guess == randomNumber){
  ...
  return (
    ...
    <View>
      ...
      > 
        <Text style={styles.buttonText}>Enter Guess</Text>
      </TouchableOpacity>
      </View>
      <Text>Total Number of Guesses: {numberOfGuess}</Text>
